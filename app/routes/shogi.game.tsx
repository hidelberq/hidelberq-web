...
import type { Route } from "./+types/shogi.game";
...
export function meta({ data: loaderData }: Route.MetaArgs) {
	...
	return [
		{ title: `将棋対戦 ${gameId} | nw-union.net` },
		{ name: "description", content: "将棋のオンライン対戦" },
	];
}
...
export default function ShogiGame({ loaderData }: Route.ComponentProps) {
	const {
		gameId,
		board,
		captured,
			...
			<header className="sticky top-0 z-10 bg-black/80 backdrop-blur-md border-b border-gray-800 px-4 py-3">
				<div className="max-w-lg mx-auto flex items-center justify-between">
					...
					<h1 className="text-lg font-bold">
						対局 <span className="text-amber-400 font-mono">{gameId}</span>
					</h1>
		...
		<div className="text-center space-y-6 p-6">
			...
			<div className="space-y-3">
				...
				<div className="flex items-center justify-center gap-3">
					<span className="text-4xl font-mono font-bold tracking-[0.3em] text-amber-400">
						{gameId}
					</span>
					<button
						type="button"
