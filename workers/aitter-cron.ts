...
// --- Configuration ---
const BATCH_SIZE = 12;
const UNSHOWN_THRESHOLD = 24; // 1日4回のみ実行のため、余裕を持った閾値に
...
- スクレイピング記事が提供されている場合は、少なくとも半数のツイートはスクレイピング記事を元ネタにすること
- 残りのツイートは最新ニュース情報を元ネタにすること
- 過去のツイートと同じ話題・内容・表現を避け、常に新鮮なツイートを生成する
- 各ツイートに元ネタとなったニュース記事のURLをsourceUrlとして含める（スクレイピング記事のURLを優先的に使用し、次に最新ニュースのURLを使用すること。URLが見つからない場合は空文字列にする）
